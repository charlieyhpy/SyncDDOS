...
def color_fade(start, end, steps):
    # start/end: tuple (R,G,B), steps: number of colors
    # returns list of 24-bit ANSI escape color codes
    gradient = []
...
class SimpleUI:
    # Color constants
    # For fading: bright yellow->orange->red
    ...
    def banner():
        ...
        # Smart vertical gradient: color_fade along the 7 lines
    ...
    @staticmethod
    def divider():
        # Gradient colored divider, fade left to right
        chars = 87  # matches length of ascii_art divider, fills terminal width more
...
def colorize_ascii_art(art, start_rgb=(255,200,60), end_rgb=(182,36,36)):
    # Applies color_fade vertically to ascii art, returns colorized string
    lines = art.strip("\n").split("\n")
...
# ------------ New: End Divider ASCII Art ------------
def print_end_divider():
    # End divider ASCII art, colored with vertical fade
    end_divider_ascii = r"""
---------------------------------------------------------------------------------------
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣠⣶⣴⣶⣶⣶⣶⣦⣤⣀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣿⣿⣿⣿⣿⣿⣿⣿⣟⣿⡻⣟⣶⣦⣄⠀⠀⠀⠀⠀⠀
...
def ddos_attack(target, total_requests, mode, target_type, port=80):
    ...
    if mode == "Fast":
        num_threads = min(200, total_requests)  # Increased from 50 to 200 for more speed
        delay = 0.0                             # From 0.01 to 0 for absolute no delay
    else:
...
"""
    ...
    # Colored divider again to match palette
    print(divline + SimpleUI.RESET + "\n")
    # ----------- END THREADS SENDING ART & DIVIDER --------------
    ...
    # ------------- Start: END ASCII ART & DIVIDER -------------
    print_end_divider()
    # ------------- End: END ASCII ART & DIVIDER ---------------
